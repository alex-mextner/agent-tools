...
REMINDER = (
    ...
    "(2) Use `subagent_type: \"fork\"` or `isolation: \"remote\"` on the Agent call — both "
    "run in the background by CC's own tool contract — or model the work as a dynamic "
    "Workflow. (`run_in_background` is NOT a real field on CC's `Agent` tool; setting it "
    "does nothing.) "
