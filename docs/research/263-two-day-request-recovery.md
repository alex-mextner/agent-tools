# Two-Day Request Recovery Report
...
| Thin AST/LSP capability layer | Report complete; task #256 still needs formal closure | https://spec-dev.hyperide.ai/spec/256-thin-ast-lsp-layer |
...
| Thin AST/LSP research | task #256, `docs/research/256-thin-ast-lsp-layer.md` | Report complete; task still needs formal closure | Close task #256 after review/commit, then create implementation tickets. |
| Non-Claude harness provisioning | tasks #250/#251 | Partly implemented elsewhere | Finish and verify rig-cli consolidation PR/worktree. |
| Codex orchestration switching | task #254 | Partly implemented | Run required brainstorm/quorum and close the Codex `pre-agent` enforcement gap or record it as unsupported with a concrete fallback. |
| Codex updater rollback | task #252 | Implemented in dirty rig-cli worktrees | Finish tests, commit, ship, and close the task. |
| Ship CI dedup/pagination | tasks #260/#262 | Mixed | Close #260 if PR #261 fully covers it; implement #262 pagination. |
...
| autonomous mode provisioning | task #130, PR #131 | Done, label may be stale | Verify issue labels/status match the closed work. |
...
| private topics / reply handling | PR #180 and tg#7857 | Done for the shipped fix, but follow-up wording was wrong | Keep task #181 for ask-once fallback behavior. |
...
| Codex usage telemetry collector | task #176 | Uncertain from task labels; backlog says closed PR, current list showed todo earlier | Verify real state and either close the label drift or implement collector. |
...
| spec.dev domain/write origin | task #144 | Blocked/partial | Resolve Cloudflare/domain/cert mismatch or standardize on working `spec-dev.hyperide.ai`. |
...
| warn after priority-changing commands | task #52, dirty worktree `task-52-priority-attention` | Forgotten/doing | Finish or park the worktree explicitly. |
