...
| `pre-monitor` **(live in CC once rig registers the `Monitor` matcher; Codex/opencode have no Monitor-equivalent tool, so this point is CC-only)** | `subagent-no-monitor` (block a SUBAGENT from calling Monitor AT ALL — Monitor is the background primitive by construction, so there's no foreground/background axis to classify like `subagent-no-bg-longproc` has; subagent-ONLY) |
