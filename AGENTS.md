...
| `pre-monitor` **(live in CC once rig registers the `Monitor` matcher; Codex/opencode have no Monitor-equivalent tool, so this point is CC-only)** | `subagent-no-monitor` (block a SUBAGENT from calling Monitor AT ALL — Monitor has no harness-tracked child and no foreground mode, unlike an ordinary backgrounded Bash command (see the caveat on the `subagent-no-bg-longproc` entry above, agent-tools#546), so there's no foreground/background axis to classify; subagent-ONLY) |
