...
- **research-cli** (scaffolded in [`tools/research-cli/`](tools/research-cli/), [#13](https://github.com/alex-mextner/agent-tools/issues/13)) — multi-provider research/panel CLI on the shared providers engine: ask a question, get a synthesized answer from a panel of models. A distinct tool from review-cli — research, not code review. To be spun out to its own repo.
