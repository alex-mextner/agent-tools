...
| `opencode_hook_bridge/` | The **`agents-hooks/v1` -> opencode bridge** - a JavaScript plugin plus Python dispatcher. The plugin is symlinked into `~/.config/opencode/plugins/`, calls the dispatcher for `tool.execute.before` / `tool.execute.after`, runs installed `~/.config/opencode/hooks/*.json` descriptors, and throws to block opencode tool calls. Maps `bash`->`pre-bash`, `edit`/`write`/`apply_patch`->`pre-write`, `task`->`pre-agent`, and completed writes->`post-write`; no `stop` mapping because opencode's session events are notification hooks, not a pre-stop block contract. stdlib-only Python + dependency-free JS. See [`opencode_hook_bridge/README.md`](opencode_hook_bridge/README.md). |
