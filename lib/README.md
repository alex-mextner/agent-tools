...
| `agenttools_dev/` | The **`dev` CLI** for project-scoped development actions: run top-level `rig.yaml` `scripts:` or `dev.e2e` jobs from the repo root, start/list/status/logs/stop configured dev servers and e2e jobs, stop only validated dev/e2e processes by target/pid/port/pgid scoped to the current repo or `DEV_PROJECT_PATHS`, and print session env exports for explicit multi-project work. stdlib-only at import; `agenttools_config`/PyYAML load lazily only when reading rig.yaml. See [`agenttools_dev/README.md`](agenttools_dev/README.md). |
