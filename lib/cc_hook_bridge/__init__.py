...
WHAT THIS IS
    A single dispatcher that Claude Code's ``settings.json`` hook entries
    ...
    CC (see agent-tools#18). This dispatcher closes that gap.
...
HOW IT'S REACHED AT RUNTIME
    rig wires one entry per event into the harness ``settings.json``:
        ...
        PostToolUse (matcher Edit|Write|MultiEdit…) → python3 -m cc_hook_bridge PostToolUse
...
INVARIANTS
    ...
    - The CC block signal is event-specific (confirmed against the live docs, CC 2.1.177):
      ...
      PostToolUse → exit 0 + top-level ``decision: "block"`` feedback;
