...
| Refusal | Why |
| ------- | --- |
| **The agent-tools checkout running THIS ship.sh is stale** | ship.sh runs straight off disk from wherever `$AGENT_TOOLS_ROOT` is checked out — no build step, so nothing keeps it synced with `origin/main` on its own (agent-tools#315). Before anything else, ship compares the checkout **actually executing this file** against its own `origin/main`: not on `main` (parked on a feature branch, or detached — the exact incident this gate was written after) → refuse with a distinct message (a plain "behind" refusal's `pull --ff-only` fix cannot succeed on a diverged branch); on `main` but behind `origin/main` **in `ci/ship/` itself** → refuse (you'd ship with an already-fixed bug still live); behind elsewhere → warn only, don't block. Disable with `SHIP_STALENESS_CHECK=0`. The periodic counterpart — [`refresh-agent-tools-checkout.sh`](./refresh-agent-tools-checkout.sh), a launchd/cron job that self-heals the checkout before anyone hits this gate — is documented in that script's own header. |
...
| `SHIP_AUDIT_FILE` | `~/.config/agent-tools/ship-audit.jsonl` | JSONL audit log; one line per non-dry-run gated ship. Review-quorum decisions: `authorized` / `bypass:approved` / `bypass:denied` / `refused` (with a `task_code`, plus `roles`/`min_roles`/`models`/`min_models` — `min_models` is `0` when the model floor was NOT enforced for that decision, since a genuinely enforced floor is always clamped to >=3 and can never legitimately be `0`; note the `bypass:approved` line is written by the shared `agenttools_hatch_escalation` lib and does not yet carry the roles/min_models fields — see the code comment at the DENIED-case call site, tracked as agent-tools#414). `--skip-ci` decisions: `skipci:bypass:approved` / `skipci:bypass:denied` / `skipci:refused` (with `gate":"skip-ci"`). `--known-flake` decisions: `confirmed` / `confirmed-local-gate-failed` / `refused` (with `"gate":"known-flake"` and the asserted check names) — `confirmed` is written only after `gh pr merge` actually succeeds, never merely after the claim checks out. |
| `SHIP_STALENESS_CHECK` | enabled | Set to `0` to disable the agent-tools checkout staleness gate (see the refusal table above). Production ops off-switch. |
| `SHIP_STALENESS_CHECK_ROOT` | (unset) | **Test-only** override for which checkout the staleness gate inspects, instead of deriving it from `$AGENT_TOOLS_ROOT` / this script's own on-disk location. Never set this in production. |
