...
| **Ticket not accepted** (`task gate <code>` exit 1) | The pre-merge **acceptance gate** (task-cli#115 / #521). When task-cli is on PATH and a ticket code is derivable (branch → PR title → PR body — the same derivation the post-merge notify uses), ship runs `task gate <code> --json` BEFORE merging and refuses while the ticket has **unchecked** criteria or criteria **checked without a proof** — printing them (`[3] survives a restart (unchecked)`) with the fix (`task accept <code>` / `task check <code> <n> … --proof …`). A ticket whose acceptance is inherently post-merge (a release publish) records `task change <code> --post-merge-acceptance "<reason>"` ON the ticket: the gate then passes and the reason lands in the audit line (`decision: authorized:post-merge-opt-out`). task-cli absent / foreign `--repo` / no derivable code → logged **skip**, never a refusal. `task gate` exit 2 for a code that WAS derived (an unknown ticket — a typo'd id, a hyphenated word like `UTF-8` read as a ticket — or a backend error) **refuses** (agent-tools#569; audit `decision: refused:unresolvable`): a PR that names a ticket task-cli cannot find is a broken PR, not an unguarded one. The refusal names the code, the exit, task-cli's own `error:` line and the source the code came from (branch / title / body), and points at the two remedies — fix the reference, or name the right ticket with `REVIEW_TASK_CODE=<code>`; a genuine backend outage is the case for the `SHIP_ACCEPTANCE_GATE=0` ops off-switch. Within one source EVERY candidate is tried in order (grammar-rejected and self-PR-rejected ones logged and skipped), so an incidental `GH-<this PR>` before the `Refs #<n>` line no longer hides the ticket (agent-tools#571). `task` is an AMBIGUOUS binary name (Taskwarrior, go-task also install as `task`) — an exit 0 OR 1 whose output does NOT carry task-cli's own JSON shape (`id`/`ok`/`criteria`) is treated as "not task-cli", not a genuine acceptance or refusal, and skips instead of blocking every merge (or, worse, silently authorizing one). Runs under `--dry-run`. Disable with `SHIP_ACCEPTANCE_GATE=0` (env, or a committed `.ship-config` line). Contract below. |
...
| `REVIEW_TASK_CODE` | (auto) | The PR's task code for the quorum gate. Unset → derived from a `HYP-<n>`/uppercase ticket token, then a purely descriptive ALL-CAPS/hyphenated code (`SME-ROADMAP-WORKTREE-NOTE`-shaped, 3+ segments, no digits), then — last — a keyword-anchored GitHub issue reference (`Fixes #105`/`Refs #105` → literal `#105`; two distinct anchored refs are ambiguous and derive nothing) or a full issue URL of the PR's own repo (`https://github.com/<owner>/<repo>/issues/105` → literal `#105`; another repo's issue, a `/pull/<n>` URL, or two distinct same-repo issues derive nothing). A Linear URL (`https://linear.app/<team>/issue/HYP-1440/…`) is caught by the `HYP-<n>` arm. Tried against the branch name then the PR body — every candidate of a source in order (a candidate with no review record moves the scan on; one with a short record refuses), and a `#<n>` / `GH-<n>` code under both spellings (agent-tools#571/#572; an explicit `REVIEW_TASK_CODE` too — `REVIEW_TASK_CODE='#565'` also finds reviews recorded as `GH-565`). None found → fail-closed refuse. |
...
ship's reading of exit 2: with a derived code it is a **refusal** (`refused:unresolvable`), not a skip — the
`error: …` line is what tells task-cli's answer apart from a foreign `task` binary's (Taskwarrior, go-task),
which still skips. `SHIP_ACCEPTANCE_GATE=0` is the ops off-switch for a real backend outage.
