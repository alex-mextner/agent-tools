...
| `REVIEW_TASK_CODE` | (auto) | The PR's task code for the quorum gate. Unset → tried against the branch name, then a `HYP-<n>` match in the PR title, then in the PR body, then a purely descriptive ALL-CAPS/hyphenated code (`SME-ROADMAP-WORKTREE-NOTE`-shaped, 3+ segments, no digits) in the title, then in the body; none found → fail-closed refuse. |
