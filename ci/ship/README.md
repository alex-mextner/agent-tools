...
| `SHIP_IMAGE_UPLOAD_CMD` | (unset) | Optional uploader for `--screenshot`: a command that takes the image (`{FILE}` token or `$1`) and prints a public URL. Without it, `--screenshot` just embeds a local-path note. A working, zero-new-credentials reference implementation: `gh-attach-image.sh --url-only {FILE}`, which uploads through GitHub's own `user-attachments/assets` endpoint (the same one the web UI uses for drag-and-drop paste) and rides the caller's existing `gh auth`. As of 2026-08 it lives at `hyperide`'s `.claude/scripts/gh-attach-image.sh` (HYP-1248/HYP-1252/HYP-1260) rather than in this repo, so a fresh machine without that repo checked out has to fetch it separately — relocating a copy into `ci/ship/` here, so `SHIP_IMAGE_UPLOAD_CMD` works out of the box for any repo using this ship script, is a known follow-up, not yet done. Wire it machine-wide (not per-repo) via `rig`'s global `env.vars` (`~/.config/rig/config.yaml`, applied with `rig apply commit --only env`) so every repo on that machine picks it up. |
